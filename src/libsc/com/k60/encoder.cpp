		...
		: m_id(id), m_count(0)
{
	...
	port_init(LIBSC_ENCODER1, IRQ_RISING | ALT1);
	gpio_ddr(LIBSC_ENCODER1, GPI);
	PortIsrManager::GetInstance()->SetIsrHandler(
			static_cast<PTX_e>(PTX(LIBSC_ENCODER1)),
			static_cast<PTn_e>(PTn(LIBSC_ENCODER1)), IsrHandler);
}
