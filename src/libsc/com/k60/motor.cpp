...
Motor::Motor(const uint8_t id, const float multiplier)
		: m_id(id), m_multiplier(multiplier)
{
	m_power = 0;
	m_is_clockwise = true;
...
{
	const uint16_t real_power = libutil::Clamp<uint16_t>(0, power * m_multiplier,
			9999);
...
{
	SetPower(libutil::Clamp<int>(0, m_power + power, 9999));
}
